...
import com.konsung.idcard.Wlt2Bmp;
...
public class IdCardUtil {
    ...
    private static int NUM_ERROR = 6;
    ...
    public IdCardUtil(String idCardNum) {
        // super();
        this.idCardNum = idCardNum.trim().toUpperCase();
    }
    ...
    public Boolean isInteger(){
        ...
        return pattern.matcher(this.idCardNum.substring(0, idCardNum.length()-1)).matches();
    }
    ...
    public int getAge() {
        ...
        String year2 = simpleDateFormat.format(new Date());
        return Integer.parseInt(year2) - Integer.parseInt(getYear());
    }
    ...
    public int isCorrect() {
        ...
        if (!this.isInteger()) {
            this.error = IdCardUtil.NUM_ERROR;
            return this.error;
        }
    ...
    public static Bitmap getImage(String idCardPic) {
        if (!TextUtils.isEmpty(idCardPic)) {
            byte[] bytes = UnitConvertUtil.hexStringToBytes(idCardPic);
            return Wlt2Bmp.getBmpFromByteArray(bytes);
        }
        return null;
    }

    public static ResidentBean createResident(String name, String idCard, String address) {
        ResidentBean info = new ResidentBean();
        info.setName(name);
        info.setIdentityCard(idCard);
        IdCardUtil util = new IdCardUtil(idCard);
        info.setSexy(util.getSexValue());
        info.setAge(util.getAge());
        info.setBirthday(util.getBirthday());
