...
public class TempChartFragment extends MVPBaseFragment<ChartContract.Presenter> implements ChartContract.View {
    ...
    protected void initView() {
        ...
        lineChart.getDescription().setText("体温趋势图");
        //设置是否可以缩放 x和y，默认true
    ...
    public void loadData() {
        mPresenter.loadChartData(resident.getPatientId(), MedicalConstant.TEMP, "", "");
    }
    ...
    private void parseData(List<HistoryData> dataList) {
        ...
        for (int i = 0; i < dataList.size(); i++) {
            ...
            for (MedicalData data : list) {
                float temp = Float.parseFloat(data.getValue());
                yValues.add(new Entry(i, temp));
                if (temp > NormalRange.TEMP_MAX) {
                    listColors.add(Color.parseColor("#FF0000"));
                } else if (temp < NormalRange.TEMP_MIN) {
        ...
        if (xValues.size() > 0)
            TempChartManager.initLineChart(getActivity(), lineChart, xValues, yValues, listColors);
    }
}
